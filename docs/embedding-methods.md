...
By default, `sneer` carries out t-SNE. But other embedding methods are
...
### `cost`, `kernel` and `normalization`

Almost all of the SNE-style probability-based embedding methods can be created
by specifying the following three arguments to `embedder`:

* `cost`: The cost function, which must be one of: 
...
* `kernel`: The kernel function used to convert squared distances to weights, 
...
* `normalization`: The normalization to be used when converting the weights to
probabilities, which must be one of:
    ...
    * `"joint"` Pairwise normalization followed by averaging (if required), to
    convert conditional probabilities into joint probabilities, as used in SSNE
    and t-SNE.

The `normalization` parameter can actually more complex input, but you don't
need to worry about it most of the time. See the section on `joint` vs `pair`
