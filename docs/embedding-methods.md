...
output:
  html_document:
    ...
    toc_float:
      collapsed: false
---
Previous: [Optimization](optimization.html). Next: [Advanced Embedding Methods](embedding-methods-advanced.html). Up: [Index](index.html).
...
If none of the pre-canned embedding methods are suitable, more control over the
embedding method is available by passing the result of calling the `embedder`
function to the `method` parameter. See the [Advanced Embedding Methods](embedding-methods-advanced.html)
