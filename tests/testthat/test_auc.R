...
pr_auc_mat <- function(dm, labels) { auc_mat(dm, labels, pr_auc_row) }
roc_auc_mat <- function(dm, labels) { auc_mat(dm, labels, roc_auc_row) }
...
test_that("random AUC PR is ~0.5", {
  expect_equal(pr_auc_mat(dm_rand, labels)$av_auc, 0.5, tolerance = 1e-2)
})
test_that("random AUC ROC is ~0.5", {
  expect_equal(roc_auc_mat(dm_rand, labels)$av_auc, 0.5, tolerance = 0.1)
})
...
test_that("perfect AUC PR is 1", {
  expect_equal(pr_auc_mat(dm_perfect, labels)$av_auc, 1.0)
})
test_that("perfect AUC ROC is 1", {
  expect_equal(roc_auc_mat(dm_perfect, labels)$av_auc, 1.0)
})

dm_anti <- matrix(seq(length.out = n * n), nrow = n)
test_that("anti-enrichment AUC PR is non-zero", {
  expect_equal(pr_auc_mat(dm_anti, labels)$label_av$`0`, 0.299,
               tolerance = 1e-2)
})
test_that("anti-enrichment AUC ROC is 0", {
  expect_equal(roc_auc_mat(dm_anti, labels)$label_av$`0`, 0.0)
})
...
test_that("AUC PR for good and bad performance", {
  pr_auc <- pr_auc_mat(dm_swap, labels)
...
test_that("AUC ROC for good and bad performance, bigger delta than PR", {
  roc_auc <- roc_auc_mat(dm_swap, labels)
...
test_that(
  ...
  pr_auc <- pr_auc_mat(dm_rand, labels)
...
test_that("random AUC ROC imbalanced classes is still 0.5", {
  roc_auc <- roc_auc_mat(dm_rand, labels)
...
test_that("anti-enrichment AUC PR is non-zero and larger for more imbalanced classes", {
  expect_equal(pr_auc_mat(dm_anti, labels)$label_av$`0`, 0.533,
               tolerance = 1e-2)
})
test_that("anti-enrichment AUC ROC is 0", {
  expect_equal(roc_auc_mat(dm_anti, labels)$label_av$`0`, 0.0)
